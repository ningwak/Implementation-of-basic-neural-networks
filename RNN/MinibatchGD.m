function [RNN, GDParams] = MinibatchGD(RNN, X, Y, GDParams, A, H, P)

Grads = ComputeGradients(RNN, X, Y, P, H, A);
for f = fieldnames(Grads)'
    Grads.(f{1}) = max(min(Grads.(f{1}), 5), -5);
end
eps = 1e-8;
for f = fieldnames(RNN)'
    GDParams.M.(f{1}) = GDParams.M.(f{1}) + Grads.(f{1}) .^ 2;
    RNN.(f{1}) = RNN.(f{1}) - GDParams.eta * Grads.(f{1}) ./ sqrt(GDParams.M.(f{1}) + eps);
end
